...
본 프로젝트는 아래와 같이 철저히 단계를 밟아가며 구조적이고 안정적으로 개발되었습니다.

**1. 프로젝트 환경 설정 및 뼈대 구축 (1단계)**
- 역할 분리를 위해 `main.py`(메뉴), `quiz.py`(데이터모델), `game.py`(게임조종통제) 등 3개의 핵심 스크립트 파일을 분할 생성했습니다.
- 사용자에게 보여줄 1~6번까지의 터미널 텍스트 UI 메뉴를 구성하고, 문자와 빈칸 등 잘못된 입력을 무한 루프(`while True`)로 방어하는 1차 예외 처리 틀을 마련했습니다.

**2. 클래스 및 모델 정의 (2단계)**
- 개별 퀴즈 데이터를 담당하는 `Quiz` 클래스를 선언하고, 향후 JSON 포맷팅 저장을 위한 헬퍼 역할을 할 `to_dict()` 구조를 정의했습니다.
- 모든 게임 흐름을 중앙 통제할 `QuizGame` 클래스의 기본 뼈대를 수립했습니다.

**3. 핵심 기능 I : JSON 데이터 영속성 부여 (3단계)**
- 프로그램이 꺼져도 퀴즈 목록이 유지되도록, `state.json` 파일을 열고 쓰고 닫는 `load_state()` 및 `save_state()` 입출력 로직을 `game.py`에 가장 먼저 탑재했습니다.

**4. 핵심 기능 II : 퀴즈 추가 및 삭제 (4단계)**
- 사용자가 직접 문제/선택지/정답 번호를 입력하여 `Quiz` 인스턴스를 만들고 영구 저장하는 로직을 작성했습니다.
- 불필요한 번호를 배열에서 도려내는(`del` 또는 `pop`) 퀴즈 삭제 기능도 연동시켰습니다.

**5. 핵심 기능 III : 퀴즈 풀기 기초 로직 (5단계)**
- 보유한 퀴즈를 반복문(`for`)으로 순회하며 뿌리고, 사용자 입력값과 실제 답을 비교하여 실시간으로 정답 점수를 누적하는 뼈대 기능을 구현했습니다.

**6. 보너스 심화 기능 강화 (6단계)**
- **랜덤 출제 및 개수 제어**: `random.shuffle()` 모듈을 도입해 문제를 섞고, 파이썬 리스트 슬라이싱 기법(`[:count]`)을 써서 원하는 개수만큼 잘라서 출제하도록 확장했습니다.
- **힌트 및 차등 배점 시스템**: 문제 출제 시 `0`번을 누르면 저장된 힌트 문구를 보여주며, 기존 10점 만점 시퀀스에서 5점 감점을 매기는 복합 분기점(`if`) 로직을 추가했습니다.
- **다이나믹 히스토리**: `datetime` 내장 라이브러리로 풀이 발생 시점의 날짜/시간을 기록하여, 최근 5차례 플레이 전적을 `state.json`의 `history` 배열로 관리 및 출력하는 스코어보드를 완성했습니다.

**7. 비상 안전장치 및 최적화 마무리 (7단계)**
- 파일을 사용자가 임의로 삭제하거나 터미널을 강종(`Ctrl+C` / `EOFError`)하는 극한의 스트레스 상황을 재현하며 방어형 예외 처리(`try/except`)를 덧칠했습니다.
- `JSONDecodeError` 발생 시 즉각 `data.py`의 더미 데이터를 끌어와 데이터베이스를 스스로 초기화 및 복구해 내는 자가 회복망을 탑재하여 완벽한 버전을 완성해 냈습니다!
